...
class Records(): 
    ...
    def read_Customers(self):
        try:
            ...
            print("customers.txt file is not exist in this folder")
            exit(0) 
        ...
        try:
            with open("products.txt", "r") as file:   
                for line in file:
                    ...
                    if len(fields) == 4:
                        Id, Name, Price, Stock = fields
                        # Attempt to convert Price and Stock to float and int, respectively
                        try:
                            Price = float(Price)
                            Stock = int(Stock)
                        except ValueError:
                            print(f"Error: Invalid numeric values in line: {line}")
                            continue  # Skip this line and proceed to the next one

                        # Create the Product object
                        productss = Product(Id, Name, Price, Stock)
                        self.Prod_list.append(productss)
                    else:
                        print(f"Error: Invalid line format in products.txt: {line}")
        except FileNotFoundError:
            print("Error: products.txt file is not found in the current working directory.")
            exit(0)
